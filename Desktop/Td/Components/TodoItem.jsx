...
const TodoItem = ({task}) => {
  ...
  return (
    <TouchableOpacity   onPress={()=> navigator.navigate("detail",{ id : task.id })}  style={tw `h-20 p-2 flex-row flex items-center rounded-lg w-full bg-white mt-4 border-2 border-gray-100`}>
        { task.status === 0 ?     <Icon type='ionicon' color="red" size={40} name='close-circle' /> : task.status === 1 ?   <Icon type='ionicon' color="yellow" size={40} name='time' />  :  <Icon type='ionicon' color="red" size={40} name='checkmark' /> }
